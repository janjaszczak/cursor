...
Same pattern as Neo4j: a backing container (`searxng`, from the public `searxng/searxng` image) must be started once, separately from Cursor, before the `searxng` MCP entry in `mcp.json` can connect to it.
