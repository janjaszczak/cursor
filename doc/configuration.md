...
**Runbook (add/rotate API keys, KeePass → `.env`):** **[doc/mcp-secrets.md](mcp-secrets.md)** — start here when adding a new key.
...
- `NEO4J_USERNAME`, `NEO4J_PASSWORD`, `NEO4J_DATABASE` — Memory MCP (launcher loads `.env`)
- `GITHUB_PERSONAL_ACCESS_TOKEN`, `GRAFANA_URL`, `GRAFANA_API_KEY`, `POSTMAN_API_KEY`, `PERPLEXITY_API_KEY`, `CONTEXT7_API_KEY` — as needed for each MCP
- `SEARXNG_URL` — fixed in `mcp.json` (`http://searxng:8080`); `SEARXNG_SECRET` optional — see [docker/mcp-searxng/README.md](../docker/mcp-searxng/README.md)

**Memory MCP credentials:** do **not** point `args` at `${userHome}/.cursor/scripts/...` on Windows — Cursor expands that to invalid `C:\c:\Users\...`. Use `python -c` + `Path.home()` (see `mcp.json`). The launcher loads `~/.cursor/.env` when `NEO4J_PASSWORD` is unset.
...
**WSL note:** `/home/<user>/.cursor` and `/mnt/c/Users/<user>/.cursor` are often **two separate trees**. Keep them in sync (copy `mcp.json`, `.env`, `scripts/mcp-run-memory.py`, `scripts/mcp-run-context7.py`), or symlink WSL `~/.cursor` to the Windows path, or set `CURSOR_CONFIG_DIR` to the canonical tree. See [mcp-secrets.md](mcp-secrets.md).
...
MCP API keys: full steps in **[mcp-secrets.md](mcp-secrets.md)**.

**Get GitHub token (shell one-off):**
```bash
GITHUB_TOKEN=$(~/.cursor/scripts/get-keepass-secret.sh "GitHub Token" "Token")
export GITHUB_TOKEN
```

**Get Context7 key for `.env`:**
```bash
~/.cursor/scripts/get-keepass-secret.sh "API Keys/Context7" "Password"
# → set CONTEXT7_API_KEY=… in ~/.cursor/.env (both WSL and Windows trees if you use both)
```
...
- **MCP API keys → `.env`:** [doc/mcp-secrets.md](mcp-secrets.md)
- **KeePass runbook:** [doc/keepass.md](keepass.md)
- **Skill:** [`skills/keepass/SKILL.md`](../skills/keepass/SKILL.md)
- **Scripts:** `get-keepass-secret.sh`, `save-keepass-password-to-keyring.sh`
